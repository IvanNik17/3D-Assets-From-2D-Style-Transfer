...
#%%
PATH_CHK = "./norm/norm_net_epoch_100.pth"
DIR_EVAL = "./textures/"
...
def testSSIM(net, in_folder, test_folder):
    ...
    with torch.no_grad():
        for data, compare in zip(inloader, testloader): 
            ...
            # print("----------")
            # im = Image.fromarray(compare_arr)
            # im.show()

            # im2 = Image.fromarray(img_out_arr)
            # im2.show()
            (score, diff) = structural_similarity(img_out_arr, compare_arr, full=True, multichannel = True)
...
def main():
    ...
    # test_folder = os.path.join(DIR_EVAL, "normal")
    # testSSIM(norm_net, input_folder, test_folder)
